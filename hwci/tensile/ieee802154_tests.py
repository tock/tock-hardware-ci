...
def radio_rx_test(boards):
    ...
    # Run test for 10 seconds and gather results. We "launch" a dummy tx
    # test to restart the board that will transmit packets.
    board_tx.run_test(1)
    test_rx_results = board_rx.run_test(10)
    ...
    # app prints "Transmitted successfully" for each successful transmission.
    success_passed = 0
    TOTAL_PACKETS = 40
    ...
    # Check if 90% of packets were transmitted successfully.
    if success_passed / TOTAL_PACKETS >= 0.80:
        board_rx.log_info("PASSED: radio_rx test")
    else:
        raise Exception("FAILED: radio_rx test -- {} out of {} packets transmitted successfully.".format(success_passed, TOTAL_PACKETS))

def radio_tx_raw_test(boards):
    # We require 2 boards for this test.
    # TODO: Better handling/reporting of case w/o at least 2 boards.
    if len(boards) < 2:
        raise Exception("Error: [Inadequate resources] - radio_rxtx test requires at least two available boards.")
    ...
    # Check if 90% of packets were transmitted successfully.
    if success_passed / TOTAL_PACKETS >= 0.90:
        board_rx.log_info("PASSED: radio_tx_raw test")
    else:
        raise Exception("FAILED: radio_tx_raw test -- {} out of {} packets transmitted successfully.".format(success_passed, TOTAL_PACKETS))

def radio_tx_test(boards):
