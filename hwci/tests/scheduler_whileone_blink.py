...
class SchedulerWhileoneBlinkTest(OneshotTest):
    ...
    def oneshot_test(self, board):
        ...
        # Map the LEDs according to board_descriptor pin mappings
        led_pins = {
            "LED1": gpio.pin("P0.13"),
            "LED2": gpio.pin("P0.14"),
        }
        ...
        for led, toggle_count in toggle_counts.items():
            ...
            assert (
                prev_toggles is None or toggle_count < prev_toggles
        ...
        serial.write(b"list\r\n")
        process_list_re = r"PID[ \t]+ShortID[ \t]+Name[ \t]+Quanta[ \t]+Syscalls[ \t]+Restarts[ \t]+Grants[ \t]+State\r\n(.*)\r\n(.*)\r\n"
        ...
        [whileone_proc, blink_proc] = (
            ...
            if "whileone" in process_list_match.group(1)
            else [process_list_match.group(2), process_list_match.group(1)]
        )
