...
class RadioTxRawTest(TestHarness):
    ...
    def test(self, boards):
        ...
        while i < len(rx_output):
            # Check for complete packet reception pattern (different offset for raw packets)
            if (i + 8 < len(rx_output) and
                "Received packet with payload of 60 bytes from offset 18" in rx_output[i] and
                ...
                "Packet destination address: 0xffff" in rx_output[i+6] and
                "Packet source PAN ID: 0xabcd" in rx_output[i+7] and
                "Packet source address: 00 00 00 00 00 00 00 00" in rx_output[i+8]):
                success_count += 1
                i += 9
            else:
                i += 1
